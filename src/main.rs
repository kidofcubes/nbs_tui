...
use editor::{AppResult, handle_key_events};
...
fn play_song(song : Song){
    ...
    let mut tick_length : f64 = (100000000_f64/(song.header.tempo as f64)) as f64;
    let mut tick_duration : Duration = std::time::Duration::from_micros(tick_length as u64);
    let mut tick: i32=-1;
    let unaccuracy = Duration::new(0, 5*1000*1000); // 5 ms untrustworthyness
    loop {
...
// fn main() -> AppResult<()>{
fn main() {
	let mut f = File::open("songs/Nyan Cat.nbs").unwrap();
    let mut buffer = vec!();
    ...
    // let mut tui = editor::Tui::new(terminal, events);
    // tui.init()?;
